...
export function Historico() {
  ...
  useEffect(() => {
    ...
    if (historicoCookie) {
      try {
        setHistorico(
          JSON.parse(decodeURIComponent(historicoCookie.split("=")[1]))
        );
      } catch {
        setHistorico([]);
      }
    ...
    >
        ...
        <ul style={{ width: "100%", padding: 0, overflowY: "auto", flex: 1 }}>
          {historico.map((item, idx) => (
            <li key={idx} className="historico-item">
              <div className="infos">
                <span className="historico-label">Nome:</span>{" "}
                <span className="historico-value">{item.nome || "-"}</span> |
                <span className="historico-label">Idade:</span>{" "}
                <span className="historico-value">{item.idade}</span> |
                <span className="historico-label">Glicose:</span>{" "}
                <span className="historico-value">{item.glicose}</span> |
                <span className="historico-label">IMC:</span>{" "}
                <span className="historico-value">{item.imc}</span> |
                <span className="historico-label">Pressão:</span>{" "}
                <span className="historico-value">{item.pressao}</span> |
                <span className="historico-label">Avaliação:</span>{" "}
                <span className="historico-avaliacao">
                  {(item.avalicao || item.avaliacao || "-").replace(
                    /^\d+\.\s*/,
                    ""
                  )}
                </span>
      ...
      {emailModal.open && emailModal.idx !== null && (
        <div className="modal-overlay">
          <div
            className="modal-resultado modal-resultado-animado"
            style={{ maxWidth: 400 }}
          >
            <button
              className="modal-close"
              onClick={() => setEmailModal({ open: false, idx: null })}
            >
              &times;
            </button>
            ...
            <form
              onSubmit={async (e) => {
                ...
                const body = {
                  ...
                  avaliacao: (item.avalicao || item.avaliacao || "-").replace(
                    /^\d+\.\s*/,
                    ""
                  ),
                  email_medico: email,
            ...
            >
              <label style={{ fontWeight: "bold" }}>E-mail que deseja enviar:</label>
              <input
                name="email_medico"
                type="email"
                required
            ...
            {emailStatus.success && (
              <div style={{ marginTop: 24, textAlign: "center" }}>
                <p className="sucesso" style={{ fontWeight: "bold" }}>
                ...
                <button
                  ...
                  style={{ marginTop: 8 }}
                  onClick={() => {
                    setEmailModal({ open: false, idx: null });
                    setEmailStatus({
                      loading: false,
                      success: null,
                      error: null,
            ...
            {emailStatus.success === false && (
              <div style={{ marginTop: 24, textAlign: "center" }}>
                <p style={{ color: "red", fontWeight: "bold" }}>
                  {emailStatus.error || "Erro ao enviar e-mail."}
                </p>
                <button
                  className="enviar-email-btn"
                  style={{ marginTop: 8 }}
        ...
        <div className="modal-overlay">
          ...
          >
            ...
            <p style={{ marginBottom: 16 }}>
              Ao clicar em "Enviar análise melhorada", você poderá encaminhar
              para o seu e-mail uma análise clínica detalhada, incluindo os
              dados do paciente e uma explicação aprofundada do motivo da
              resposta da IA.
            </p>
            <button
              className="enviar-email-btn"
              onClick={() => {
                setEmailModal({ open: true, idx: infoModal });
                setInfoModal(null);
              }}
              style={{ width: "100%" }}
            >
